...
bool	PatchJumpLong( CJumpLocation jump, CCodeLabel target )
{
	...
	if( op_code.op == static_cast<u32>(OpCodeValue::J) || op_code.op == static_cast<u32>(OpCodeValue::JAL) )
	{
	...
	else
	{
		...
		bool	is_standard_branch( op_code.op == static_cast<u32>(OpCodeValue::BNE) || op_code.op == static_cast<u32>(OpCodeValue::BEQ) ||
									op_code.op == static_cast<u32>(OpCodeValue::BEQL) || op_code.op == static_cast<u32>(OpCodeValue::BNEL) ||
									op_code.op == static_cast<u32>(OpCodeValue::BLEZ) || op_code.op == static_cast<u32>(OpCodeValue::BGTZ) );
		bool	is_regimm_branch(  (op_code.op == static_cast<u32>(OpCodeValue::REGIMM) && (op_code.regimm_op == RegImmOp_BGEZ ||
																op_code.regimm_op == RegImmOp_BLTZ ||
																op_code.regimm_op == RegImmOp_BGEZL ||
																op_code.regimm_op == RegImmOp_BLTZL ) ) );
		bool	is_cop1_branch( (op_code.cop1_op == Cop1Op_BCInstr) && ( op_code.cop1_bc == ECop1BCOp::BC1F  ||
																		 op_code.cop1_bc == ECop1BCOp::BC1FL ||
																		 op_code.cop1_bc == ECop1BCOp::BC1T  ||
																		 op_code.cop1_bc == ECop1BCOp::BC1TL ) );
